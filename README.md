...
# MicroPythonでの使い方
examples/for_micropython.pyにMicroPython環境用のサンプルコード(Raspberry Pi Pico用)があります。Raspberry Pi Picoではそのまま使えます。ESP32ではピンを変えれば動きます。
...
# バッファを大きく確保
uart = machine.UART(0, baudrate=115200, tx=machine.Pin(0), rx=machine.Pin(1), rxbuf=2048)# 環境に合わせる
gnss = pygps2(op4=False)# decimal機能を無効化
...
- ボーレート 9600,115200,460800(ボードの処理性能による)
#
