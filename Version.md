...
# Version 3.96
- 割り込み関数を呼べるようにした。op7で呼び出される関数を設定できる。
- BD GNという衛生識別子対応していなかった問題を修正。

# Version 3.95
- チェックサム検証が使われていなかった問題を修正した。op4で設定できる。
- op6を追加し、はじめのセンテンスを選択できるようにした。これにより、センテンスの順番が変わっても、正しくGSVやGSAを解析できるようになった。
- GGAを解析したとき、すべての解析変数をリセットしないようにした。
